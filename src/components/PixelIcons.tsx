// Monochrome pixel-art SVG icons — all use fill="currentColor"

export function IconProfile({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      <rect x="5" y="1" width="6" height="1" fill="currentColor"/>
      <rect x="4" y="2" width="8" height="1" fill="currentColor"/>
      <rect x="4" y="3" width="8" height="3" fill="currentColor"/>
      <rect x="5" y="6" width="6" height="1" fill="currentColor"/>
      <rect x="6" y="7" width="4" height="1" fill="currentColor"/>
      <rect x="3" y="9" width="10" height="1" fill="currentColor"/>
      <rect x="2" y="10" width="12" height="1" fill="currentColor"/>
      <rect x="1" y="11" width="14" height="4" fill="currentColor"/>
    </svg>
  )
}

export function IconTools({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      <rect x="1" y="1" width="2" height="2" fill="currentColor"/>
      <rect x="3" y="2" width="5" height="1" fill="currentColor"/>
      <rect x="8" y="1" width="2" height="2" fill="currentColor"/>
      <rect x="2" y="3" width="2" height="1" fill="currentColor"/>
      <rect x="1" y="5" width="3" height="8" fill="currentColor"/>
      <rect x="6" y="3" width="2" height="1" fill="currentColor"/>
      <rect x="5" y="5" width="6" height="1" fill="currentColor"/>
      <rect x="4" y="6" width="8" height="1" fill="currentColor"/>
      <rect x="4" y="7" width="8" height="5" fill="currentColor"/>
      <rect x="5" y="12" width="6" height="1" fill="currentColor"/>
      <rect x="6" y="13" width="4" height="1" fill="currentColor"/>
    </svg>
  )
}

export function IconNormies({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      {/* Stylized "n" — the Normies brand mark */}
      {/* Top horizontal bar */}
      <rect x="5"  y="1"  width="10" height="3"  fill="currentColor"/>
      {/* Right vertical leg */}
      <rect x="12" y="1"  width="3"  height="14" fill="currentColor"/>
      {/* Left vertical leg (starts lower) */}
      <rect x="1"  y="3"  width="3"  height="12" fill="currentColor"/>
      {/* Inner foot rectangle */}
      <rect x="7"  y="7"  width="3"  height="8"  fill="currentColor"/>
    </svg>
  )
}

export function IconKhorafun({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      <rect x="7" y="1" width="2" height="2" fill="currentColor"/>
      <rect x="5" y="3" width="6" height="1" fill="currentColor"/>
      <rect x="3" y="4" width="2" height="2" fill="currentColor"/>
      <rect x="11" y="4" width="2" height="2" fill="currentColor"/>
      <rect x="2" y="6" width="2" height="4" fill="currentColor"/>
      <rect x="12" y="6" width="2" height="4" fill="currentColor"/>
      <rect x="3" y="10" width="2" height="2" fill="currentColor"/>
      <rect x="11" y="10" width="2" height="2" fill="currentColor"/>
      <rect x="5" y="12" width="6" height="1" fill="currentColor"/>
      <rect x="7" y="13" width="2" height="2" fill="currentColor"/>
      <rect x="6" y="6" width="4" height="4" fill="currentColor"/>
    </svg>
  )
}

export function IconContact({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      <rect x="1" y="3" width="14" height="1" fill="currentColor"/>
      <rect x="1" y="4" width="1" height="8" fill="currentColor"/>
      <rect x="14" y="4" width="1" height="8" fill="currentColor"/>
      <rect x="1" y="12" width="14" height="1" fill="currentColor"/>
      {/* envelope V */}
      <rect x="2" y="5" width="2" height="1" fill="currentColor"/>
      <rect x="12" y="5" width="2" height="1" fill="currentColor"/>
      <rect x="4" y="6" width="2" height="1" fill="currentColor"/>
      <rect x="10" y="6" width="2" height="1" fill="currentColor"/>
      <rect x="6" y="7" width="2" height="1" fill="currentColor"/>
      <rect x="8" y="7" width="2" height="1" fill="currentColor"/>
      <rect x="7" y="8" width="2" height="1" fill="currentColor"/>
    </svg>
  )
}

export function IconTwitter({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      {/* X / Twitter logo pixel style */}
      <rect x="2" y="2" width="2" height="2" fill="currentColor"/>
      <rect x="4" y="4" width="2" height="2" fill="currentColor"/>
      <rect x="6" y="6" width="2" height="2" fill="currentColor"/>
      <rect x="8" y="6" width="2" height="2" fill="currentColor"/>
      <rect x="10" y="4" width="2" height="2" fill="currentColor"/>
      <rect x="12" y="2" width="2" height="2" fill="currentColor"/>
      <rect x="4" y="8" width="2" height="2" fill="currentColor"/>
      <rect x="2" y="10" width="2" height="2" fill="currentColor"/>
      <rect x="10" y="8" width="2" height="2" fill="currentColor"/>
      <rect x="12" y="10" width="2" height="2" fill="currentColor"/>
      <rect x="12" y="12" width="2" height="2" fill="currentColor"/>
    </svg>
  )
}

export function IconCoffee({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      {/* Steam wisps */}
      <rect x="4"  y="0" width="1" height="2" fill="currentColor"/>
      <rect x="3"  y="2" width="1" height="1" fill="currentColor"/>
      <rect x="7"  y="0" width="1" height="2" fill="currentColor"/>
      <rect x="8"  y="2" width="1" height="1" fill="currentColor"/>
      <rect x="10" y="0" width="1" height="2" fill="currentColor"/>
      <rect x="11" y="2" width="1" height="1" fill="currentColor"/>
      {/* Cup top edge */}
      <rect x="1"  y="4"  width="11" height="1" fill="currentColor"/>
      {/* Cup walls */}
      <rect x="1"  y="5"  width="1"  height="7" fill="currentColor"/>
      <rect x="11" y="5"  width="1"  height="7" fill="currentColor"/>
      {/* Cup bottom */}
      <rect x="1"  y="12" width="11" height="1" fill="currentColor"/>
      {/* Handle (C-shape) */}
      <rect x="12" y="6"  width="2"  height="1" fill="currentColor"/>
      <rect x="13" y="7"  width="1"  height="2" fill="currentColor"/>
      <rect x="12" y="9"  width="2"  height="1" fill="currentColor"/>
      {/* Saucer */}
      <rect x="0"  y="14" width="14" height="1" fill="currentColor"/>
    </svg>
  )
}

export function IconLine({ size = 48 }: { size?: number }) {
  return (
    <img
      src="/line.png"
      alt="ATB / LINE"
      width={size}
      height={size}
      style={{
        imageRendering: 'pixelated',
        display: 'block',
        width: size,
        height: size,
        objectFit: 'cover',
      }}
    />
  )
}

export function IconWorks({ size = 48 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ imageRendering: 'pixelated' }}>
      {/* Diamond / gem shape */}
      <rect x="6"  y="1"  width="4" height="1" fill="currentColor"/>
      <rect x="4"  y="2"  width="2" height="1" fill="currentColor"/>
      <rect x="10" y="2"  width="2" height="1" fill="currentColor"/>
      <rect x="2"  y="3"  width="2" height="1" fill="currentColor"/>
      <rect x="12" y="3"  width="2" height="1" fill="currentColor"/>
      <rect x="1"  y="4"  width="1" height="2" fill="currentColor"/>
      <rect x="14" y="4"  width="1" height="2" fill="currentColor"/>
      <rect x="2"  y="6"  width="12" height="1" fill="currentColor"/>
      <rect x="3"  y="7"  width="2" height="2" fill="currentColor"/>
      <rect x="7"  y="7"  width="2" height="2" fill="currentColor"/>
      <rect x="11" y="7"  width="2" height="2" fill="currentColor"/>
      <rect x="5"  y="9"  width="2" height="2" fill="currentColor"/>
      <rect x="9"  y="9"  width="2" height="2" fill="currentColor"/>
      <rect x="6"  y="11" width="4" height="2" fill="currentColor"/>
      <rect x="7"  y="13" width="2" height="2" fill="currentColor"/>
    </svg>
  )
}
